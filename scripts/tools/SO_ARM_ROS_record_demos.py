...
def pre_process_actions(
    ...
    Args:
        teleop_data: Data from the teleoperation device (6D pose delta, gripper command).
        num_envs: Number of environments.
        device: Device to create tensors on.

    Returns:
        Processed actions as a tensor (6D for SO100 - no separate gripper dimension).
    """
    # Extract pose delta and gripper command from keyboard
    delta_pose, gripper_command = teleop_data
    
    # Convert to torch tensor - only 6D for SO100 (no separate gripper)
    actions = torch.tensor(delta_pose, dtype=torch.float, device=device).repeat(num_envs, 1)
